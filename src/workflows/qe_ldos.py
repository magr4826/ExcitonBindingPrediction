...
# workflow directory
wf_dir = os.path.join(os.getcwd(), "qe_ldos")
...
vbm_ldos, structure = qe_runner.qe_ldos_run(calc_data_conv, vbm - 1, vbm)
cbm_ldos, structure = qe_runner.qe_ldos_run(calc_data_conv, cbm, cbm + 1)

# save the atomic site and the corresponding number of localized above-/below-gap states
with open(f"{id}_ldos.csv", "w") as outfile:
    for idx in range(len(structure.labels)):
        outfile.write(
            " ".join(
                [structure.labels[idx], str(vbm_ldos[idx]), str(cbm_ldos[idx]), "\n"]
            )
        )
